...
open class AZDropdownMenu: UIView {
    ...
    fileprivate var calcMenuHeight : CGFloat {
        get {
            return CGFloat(Double(itemHeight * itemDataSource.count) + 50.0)
        }
    }
...
extension AZDropdownMenu: UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = getCellByData(cellIndex: indexPath.item) {
    ...
    func getCellByData(cellIndex: Int) -> AZDropdownMenuBaseCell? {
        if let _ = itemDataSource[cellIndex].icon {
